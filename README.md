...
| [Jev Plays Doom](https://github.com/tirukovelamanoj/jev-plays-doom) | Doom played by a System One model that never sees the screen: monster bearings in as structured state, one of three buttons out, 212ms per decision. Handed the firing rule it matches hand-coded logic exactly, 6.55 kills each, agreeing 30 times out of 30. Asked to judge for itself when a shot is lined up, it never fires once and dies with all 26 bullets — while its confidence honestly falls from 0.88 to 0.41. A fast evaluator of boundaries you supply, not a discoverer of them. | Python, TypeSafe AI, Jev, System One, ViZDoom |
