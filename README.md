...
| -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| [Unswattable](https://fly.manojtirukovela.com) · [src](https://github.com/tirukovelamanoj/unswattable)                   | A fruit fly dodges your cursor using its real connectome. 1,984 neurons and 17,716 synapses from the FlyWire brain map, run as a leaky integrate-and-fire network live in the browser. Nothing sits between the spikes and the movement: looming detectors converge on the giant fiber and descending neurons move the fly. Lesion buttons silence a population mid-game and it goes blind to you. The mushroom body conditions on the direction you keep striking from, using the fly's own dopamine-gated plasticity rule across 3,988 real synapses. | JavaScript, Three.js, WebGL, Python, NumPy |
...
| [Jev Plays Doom](https://github.com/tirukovelamanoj/jev-plays-doom) | Doom played by Jev from TypeSafe AI, trained with Reinforcement Learning for Calibrated Decisions (RLCD), which returns typed decisions with a confidence value rather than text. It never sees the screen: monster bearings in as structured state, one of three buttons out, 212ms per decision. Handed the firing rule it matches hand-coded logic exactly, 6.55 kills each, agreeing 30 times out of 30. Asked to judge for itself when a shot is lined up, it never fires once and dies with all 26 bullets, while its confidence falls from 0.88 to 0.41. A fast evaluator of boundaries you supply, not a discoverer of them. | Python, TypeSafe AI Jev, RLCD, ViZDoom |
| [ATC Arena](https://game.manojtirukovela.com) · [src](https://github.com/tirukovelamanoj/ATC)      | Real-time air traffic control game a person plays with a mouse and an RL agent plays over the same public API. Fully convolutional policy trained with Proximal Policy Optimization (PPO), ~58k weights at any grid resolution, exported to ONNX so the server flies it without shipping PyTorch. Weights are downloadable, so anyone can point their own model at the live environment. | Python, PyTorch, Gymnasium, PPO, ONNX, FastAPI |
...
[![Portfolio](https://img.shields.io/badge/Portfolio-manojtirukovela.com-3b82f6?style=flat&logo=vercel&logoColor=white)](https://manojtirukovela.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-manojtirukovela-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/manojtirukovela/)
